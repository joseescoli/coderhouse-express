...
export default class UserDao {
  ...
  async getall() {
    try {
      const users = await UserModel.find({}, {_id: 1, first_name: 1, last_name: 1, email: 1, age: 1, role: 1, profileImg: 1, last_connection: 1, documents: 1})
      if( !users )
        return false
      else
        return users
  ...
  async deleteById(id){
    try {
      ...
      logger.debug( `User ID: ${id} removed!` );
      return user
    } catch (error) {
      logger.error(error.message)
    }
  ...
  async deleteManyIds(ids) {
    try {
      const response = await productsModel.deleteMany( { _id: ids } )
      logger.debug( `The following user IDs were removed: ${ids}` );
      return response;
    } catch (error) {
      logger.error(error.message)
    }
